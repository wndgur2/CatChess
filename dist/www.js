...
var require_auth = __commonJS({
  "src/routes/auth.js"(exports2, module2) {
    ...
    var redirectUri = process.env.NODE_ENV ? "http://localhost:8080/auth/google/callback" : "http://catchess.ap-northeast-2.elasticbeanstalk.com/auth/google/callback";
    ...
    var SKILLS = {
      roar: {
        ...
        execute: (cat) => {
          ...
          cat.battleField.getNearestUnits(cat, RANGE, TARGET_AMOUNT, false).forEach(({ _, target }) => {
            target.pushModifier(
              new Modifier(
                { speedRatio: 0 },
      ...
      neckBite: {
        ...
        execute: (cat) => {
          ...
          cat.battleField.getNearestUnits(
            cat,
            cat.range,
            TARGET_AMOUNT,
            getAlly = false
  ...
  "src/modules/unit/SimpleCat.js"(exports2, module2) {
    ...
    var SimpleCat = class _SimpleCat extends Unit {
      ...
      static getRandomCatType(cost) {
        let candidates;
        if (cost)
          candidates = Object.values(_SimpleCat.prototypes).filter(
            (cat) => cat.cost === cost
          );
...
var require_Player = __commonJS({
  "src/modules/Player.js"(exports2, module2) {
    ...
    var Player = class {
      ...
      checkUpgrade() {
        let catIdTierAmount = {}, checkingArea;
  ...
  "src/modules/BattleField.js"(exports2, module2) {
    ...
    var BattleField = class {
      ...
      getNextMove(cat, target) {
        let visited = [];
        this.field.forEach(
          (row) => visited.push(row.map((c) => c !== null && c.owner === cat.owner))
        );
        let queue = [];
      ...
      getNearestUnits(cat, range, amount, getAlly2 = false) {
        ...
        while (queue.length > 0) {
          ...
          if (this.field[y][x] && (getAlly2 ? this.field[y][x].owner === cat.owner : this.field[y][x].owner !== cat.owner))
  ...
  "src/modules/Battle.js"(exports2, module2) {
    ...
    var Battle = class _Battle {
      ...
      constructor(player1, player2, isCreep = false) {
        ...
        let board1 = player1.board.map(
          (row) => row.map((c) => c ? c.clone() : null)
        );
        let board2 = player2.board.map((row) => row.map((c) => c ? c.clone() : null).reverse()).reverse();
        ...
        [this.player1, this.player2].forEach((player) => {
          ...
          sendMsg(player.ws, "battleReady", {
            battleId: this.id,
            board: this.battleField.field.map(
              (row) => row.map((c) => c ? { ...c, battleField: null } : null)
            ),
      ...
      punish() {
        if (this.finished) return;
        let p1Units = this.battleField.getCats(this.player1.id).length, p2Units = this.battleField.getCats(this.player2.id).length, damage;
        if (p1Units > p2Units) {
          if (!this.isCreep) {
            this.player1._money++;
  ...
  "src/modules/Game.js"(exports2, module2) {
    ...
    var Game = class _Game {
      ...
      readyState() {
        ...
        if (this.stage == 1 && this.round <= Object.keys(CREEP_ROUNDS).length) {
          this.players.forEach((player, i) => {
            ...
            this.creeps[i].board = CREEP_ROUNDS[this.round].map(
              (row) => row.map((c) => {
                if (!c) return null;
      ...
      finishState() {
        ...
        if (isEnd)
          this.timeout = setTimeout(() => this.endState(), this.time * 1e3);
        else
          this.timeout = setTimeout(
            () => this.arrangeState(),
            this.time * 1e3
...
async function run() {
  try {
    ...
    var normalizePort = normalizePort2, onError = onError2, onListening = onListening2;
    var app = require_app();
