...
export default class Unit {
  ...
  constructor(data) {
    ...
    this.name = Unit.CATS.concat(Unit.CREEPS).find((unit) => unit.id === data.id).name
    this.desc = Unit.CATS.concat(Unit.CREEPS).find((unit) => unit.id === data.id).desc
    this.id = data.id
    this.uid = data.uid
    this.tier = data.tier
    ...
    function animateHealthDamage() {
      if (damagedHealthMesh.scale.x > healthBarMesh.scale.x) {
        ...
        damagedHealthMesh.position.x =
          ((1 - damagedHealthMesh.scale.x) * THREE_CONSTS.HEALTHBAR_WIDTH) / 2
